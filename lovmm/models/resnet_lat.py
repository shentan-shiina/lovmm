...
from lovmm.models.resnet import ConvBlock, IdentityBlock
...
class ResNet45_10s(nn.Module):
    ...
    def _make_layers(self):
        ...
        # # HABITAT ADDITIONAL LAYERS
        # self.layer_add_1024 = nn.Sequential(
        #     ConvBlock(1024, [1024, 1024, 1024], kernel_size=3, stride=1, batchnorm=self.batchnorm),
        #     IdentityBlock(1024, [1024, 1024, 1024], kernel_size=3, stride=1, batchnorm=self.batchnorm),
        # )
    ...
    def forward(self, x):
        ...
        for layer in [self.layer6, self.layer7, self.layer8, self.layer9, self.layer10, self.conv2]:
            im.append(x)
            x = layer(x)
        # for layer in [self.layer_add_1024, self.layer6, self.layer7, self.layer8, self.layer9, self.layer10, self.conv2]:
        #     im.append(x)
        #     x = layer(x)
