...
class Transport(nn.Module):
    ...
    def correlate(self, in0, in1, softmax):
        ...
        output = F.interpolate(output, size=(in0.shape[-2], in0.shape[-1]), mode='bilinear', align_corners=False)
    ...
    def correlate6d(self, in0, in1, softmax):
        """Correlate two input tensors."""
        # output = F.conv2d(in0, in1, padding=(self.pad_size, self.pad_size))
        # output = F.interpolate(output, size=(in0.shape[-2], in0.shape[-1]), mode='bilinear')
        # output = output[:,:,self.pad_size:-self.pad_size, self.pad_size:-self.pad_size]

        z_tensor = F.conv2d(in0[:, :9, :, :], in1[:, :9, :, :], padding=(self.pad_size, self.pad_size))
        # For better receptive field, theta use all the channels
        roll_tensor = F.conv2d(in0[:, 9:18, :, :], in1[:, 9:18, :, :], padding=(self.pad_size, self.pad_size))
        pitch_tensor = F.conv2d(in0[:, 18:27, :, :], in1[:, 18:27, :, :], padding=(self.pad_size, self.pad_size))

        z_tensor = F.interpolate(z_tensor, size=(in0.shape[-2], in0.shape[-1]), mode='bilinear', align_corners=False)
        ...
        # output = F.interpolate(output, size=(in0.shape[-2], in0.shape[-1]), mode='bilinear', align_corners=False)
        # output = output[:,:,self.pad_size:-self.pad_size, self.pad_size:-self.pad_size]
        # z_tensor, theta_tensor, roll_tensor, pitch_tensor = output, output, output, output

        return z_tensor, roll_tensor, pitch_tensor
