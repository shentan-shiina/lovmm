...
def pix_to_xyz_habitat(pixel, height, bounds, pixel_size, skip_height=False, scene_name = 'scene_1'):
    """Convert from pixel location on heightmap to 3D position."""
    original_H = 467
    original_W = 233
    H, W = height.shape[:2]
    u, v = pixel
    u_original = np.floor(u * original_H / H).astype(int)
    v_original = np.floor(v * original_W / W).astype(int)

    x = bounds[0, 1] - u_original * pixel_size # img is transpose to top-left
    z = bounds[2, 0] + v_original * pixel_size

    if not skip_height:
        y = bounds[1, 0] + height[u, v]
    else:
        y = 0.0
    if scene_name == 'scene_1':
        ...
        z = z - (-0.25)
        return (-z, -x, y)
    elif scene_name == 'scene_3':
        x = x - 14.25
        y = y - (-1.1)
        y = 0 if y < 0 else y
        z = z - 10.55
        return (-z, -x, y)
    elif scene_name == 'scene_4':
        x = x - 12.3
        y = y - (-0.86)
        y = 0 if y < 0 else y
        z = z - 4.15
        return (-z, -x, y)
    elif scene_name == 'habitat':
        return (x, y, z)

def xyz_to_pix_habitat(position, bounds, pixel_size):
    """Convert from 3D position to pixel location on heightmap."""
    u = int(np.round((position[2] - bounds[2, 0]) / pixel_size))
    v = int(np.round((position[0] - bounds[0, 0]) / pixel_size))
    return (u, v)
...
def eulerXYZ_to_quatXYZW_habitat(rotation):  # pylint: disable=invalid-name
    """Abstraction for converting from a 3-parameter rotation to quaterion.
...
def perturb(input_images, pixels, theta_sigma=60, add_noise=False, brightness_range=(0.8, 1.2), noise_range=(0.0, 1), blur_range=(0, 1)):
    """Data augmentation on images with random brightness, noise, and Gaussian blur."""
    input_image = input_images[0]
    if input_images[1] is not None:
        input_image_place = input_images[1]
    else:
        input_image_place = input_images[0]
    image_size = input_image.shape[:2]
    ...
    input_image_place = cv2.warpAffine(
        input_image_place,
        transform[:2, :], (image_size[1], image_size[0]),
        flags=cv2.INTER_LINEAR)
    ...
    color = np.int32(input_image[:,:,:3])
    depth = np.float32(input_image[:,:,3:])
    color_place = np.int32(input_image_place[:,:,:3])
    depth_place = np.float32(input_image_place[:,:,3:])
    # Apply random noise within the specified range
    if add_noise:
        # Apply random brightness adjustment within the specified range
        brightness_factor = random.uniform(*brightness_range)
        color = np.clip(color * brightness_factor, 0, 255)
        color_place = np.clip(color_place * brightness_factor, 0, 255)
        # depth = np.clip(depth * brightness_factor, 0, 255).astype(np.uint8) # do not process depth image for

        noise_level = random.uniform(*noise_range)
        color += np.int32(np.random.normal(0, noise_level, image_size + (3,)))
        depth += np.float32(np.random.normal(0, noise_level/1000, image_size + (3,)))
        color_place += np.int32(np.random.normal(0, noise_level, image_size + (3,)))
        depth_place += np.float32(np.random.normal(0, noise_level/1000, image_size + (3,)))

        # Apply random Gaussian blur within the specified range
        blur_sigma = random.uniform(*blur_range)
        if blur_sigma > 0:
            color = cv2.GaussianBlur(color, (0, 0), blur_sigma)
            color_place = cv2.GaussianBlur(color_place, (0, 0), blur_sigma)

    color = np.uint8(np.clip(color, 0, 255))
    color_place = np.uint8(np.clip(color_place, 0, 255))

    input_image = np.concatenate((color, depth), axis=2)
    input_image_place = np.concatenate((color_place, depth_place), axis=2)

    return (input_image,input_image_place), new_pixels, new_rounded_pixels, transform_params
...
class ImageRotator:
    ...
    def __call__(self, x_list, pivot, reverse=False):
        ...
        for i, angle in enumerate(self.angles):
            ...
            _, _, h, w = x.shape
            x_warped: torch.tensor = kornia.warp_affine(x.float().to('cuda'), M.to('cuda'), dsize=(h, w))
            x_warped = x_warped
