...
import numpy as np
import hydra
from lovmm import agents
from lovmm import dataset
from lovmm import tasks
from lovmm.utils import utils
from lovmm.environments.environment import Environment
    ...
    for ckpt in ckpts_to_eval:
        ...
        for train_run in range(vcfg['n_repeats']):
            ...
            for i in range(0, n_demos):
                ...
                env.set_task(task)
                _ = env.reset()
                # TODO: REVISE HERE TO SYNC BULLET&HABITAT simulation
                obs = episode[0][0] # broke, as obs must be current environment observation from habitat, so this code must be under viewer.py

                info = env.info
                reward = 0

                # TODO: HABITAT REVISE HERE TO create a habitat frame to bullet frame action pose converter
                for _ in range(task.max_steps):
                    act = agent.act(obs, info, goal,
                                    bounds_habitat=np.array([[0.4, 1.8], [2, 5], [-1.1, -0.4]]),
                                    # bounds_habitat=np.array([[0.6, 1.6], [2, 5], [-1.0, -0.5]]),
                                    pixel_size_habitat=0.003)
